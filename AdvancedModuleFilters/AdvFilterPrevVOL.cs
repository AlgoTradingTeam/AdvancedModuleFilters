...
// Type: TSLab.AdvancedModuleFilters.AdvFilterPrevVOL
...
namespace TSLab.AdvancedModuleFilters
{
  ...
  [HandlerName("Filter PrevVOL")]
  [Input(0, TemplateTypes.SECURITY, Name = "Источник")]
  [Description("Значение индикатора SMA (от объема) больше или меньше значения индикатора предыдущего SMA (от объема).")]
  public class AdvFilterPrevVOL : IBar2BoolsHandler, IOneSourceHandler, IBooleanReturns, IStreamHandler, IHandler, ISecurityInputs, IContextUses
  {
    [HandlerParameter(Default = "true", IsShown = false, Name = "VOL > VOL[-i]", NotOptimized = true)]
    ...
    public IList<bool> Execute(ISecurity source)
    {
      ...
      IList<double> doubleList = Series.SMA(source.Volumes, this.ParPeriod, (IMemoryContext) this.Context);
